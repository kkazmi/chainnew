    	...
    	if($new_pin==$confirm_pin)
    	{
            if($this->Auth_model->chkpasswordbyemail($old_pin)==1)
            {
            	if($this->Auth_model->updatepinpassword($old_pin,$new_pin))
                    {
                    	$this->session->set_flashdata('success', 'Your pin has been updated successfully.');
                        redirect('changepin');
                    }
                    else
                    {
                        $this->session->set_flashdata('error', 'Error occurred while change pin !!!');
                        redirect('changepin');
                    }
            }else{

            $this->session->set_flashdata('error', 'Please enter valid old pin !!!');
            redirect('changepin');
            }
        }else{
